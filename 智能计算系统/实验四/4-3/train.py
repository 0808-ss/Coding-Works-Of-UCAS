...
class VGG19(nn.Module):
    def __init__(self):
        ...
        self.layer1 = nn.Sequential(*list(a.children())[0:2])
        #TODO: 定义self.layer2为第4层卷积后对应的特征
        self.layer2 = nn.Sequential(*list(a.children())[2:5])
        #TODO: 定义self.layer3为第8层卷积后对应的特征
        self.layer3 = nn.Sequential(*list(a.children())[5:10])
        #TODO: 定义self.layer4为第12层卷积后对应的特征
        self.layer4 = nn.Sequential(*list(a.children())[10:15])
...
class TransNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.layer = nn.Sequential(
            ...
            ###################下采样层################
            # TODO：构建图像转换网络，第一层卷积
            nn.Conv2d(in_channels=3,out_channels=32,kernel_size=9,padding=4,bias=False),
            # TODO：实例归一化
...
if __name__ == '__main__':
    ...
    # print(s3.shape)
    # print(s4.shape)
    # f_map = torch.cat([s1,s2,s3,s4],dim=0)
    # gram_matrix = get_gram_matrix(f_map)
    # gram_matrix.detach_()
    # s1,s2,s3,s4 = gram_matrix.unbind(0)
    ...
    while j <= epochs:
        for i, image in enumerate(data_loader):
            ...
            loss_s1, loss_s2, loss_s3, loss_s4 = loss_func(gram_out1,s1), loss_func(gram_out2,s2), loss_func(gram_out3,s3), loss_func(gram_out4,s4)
            ...
            if i % 10 == 0:
                ...
                image = torch.cat([image_g, image_c], dim=3)
                save_image(image,'./out/train/img.jpg')
        j += 1
