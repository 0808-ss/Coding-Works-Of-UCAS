           ...
           for idxc in range(self.channel_out):
               for idxh in range(height_out):
                   for idxw in range(width_out):
                       ...
                       self.output[idxn, idxc, idxh, idxw] = self.bias[idxc] + np.sum(np.multiply(self.weight[:,:,:,idxc], \
                                                                                                  self.input_pad[idxn, :, \
                                                                                                  idxh*self.stride:idxh*self.stride+self.kernel_size, \
                                                                                                  idxw*self.stride:idxw*self.stride+self.kernel_size]))
                        # 循环的解析：
                        # self.weight[:,:,:,idxc] : 代表一个卷积核
                        # self.input_pad[idxn, :, idxh*self.stride:idxh*self.stride+self.kernel_size, idxw*self.stride:idxw*self.stride+self.kernel_size]
                        # 这说白了就是一个和卷积核大小相同的特征图，这个特征图的位置是self.input_pad的第idxn个样本，第idxh行，第idxw列开始的一个卷积核大小的特征图
       return self.output
