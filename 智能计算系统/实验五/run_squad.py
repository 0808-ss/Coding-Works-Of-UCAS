...
def train(args, train_dataset, model, tokenizer):
    ...
    # len(train_dataloader) 的值等于：⌈数据集大小 ÷ 批量大小⌉，即数据集中的样本总数除以批量大小，向上取整。
    train_dataloader = DataLoader(train_dataset,sampler=train_sampler, batch_size=args.train_batch_size)

    
    if args.max_steps > 0:
        # 直接使用指定的 max_steps 作为总训练步数
        # 重新计算训练轮数 num_train_epochs
        # args.num_train_epochs 训练的总轮数（epochs）
        # args.gradient_accumulation_steps 梯度累积步数
        # args.max_steps (总更新次数) 
    ...
    # 预热阶段：开始时使用较小的学习率，逐渐增加到设定值，有助于模型稳定训练
    # 线性衰减：预热后，学习率会从设定值线性降低到接近于0，帮助模型收敛到更好的局部最优解
    ...
    if args.fp16:
        try:
            ...
            # 这行代码创建了PyTorch中用于混合精度训练的梯度缩放器(Gradient Scaler)实例
            scaler = GradScaler()
    ...
    # 在PyTorch中，当执行反向传播（backward pass）时，梯度会累积到参数的.grad属性中。
    # 如果不清零梯度，那么每次执行loss.backward()时，新计算的梯度会与之前的梯度相加，而不是替换它们。
    model.zero_grad()
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            batch = tuple(j.to(args.device) for j in batch)
            ...
            if args.n_gpu > 1:
                ...
                # 每个批次被发送到不同的 GPU 上并行处理
                # 每个 GPU 计算自己的损失值
                # 返回的 loss 是一个包含所有 GPU 上损失值的张量
            if args.gradient_accumulation_steps > 1:
                # 梯度累积是一种在内存受限情况下模拟大批量训练的技术
                # 不是每次前向传播后都更新模型，而是累积多次迭代的梯度，然后一次性更新
                # 如果批量大小为 B，累积步数为 N，则等效批量大小为 B×N

                # 这个除法就是为了在梯度累积的情况下，维持与正常批量训练相同的数学等价性：
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                # 这行代码用于梯度裁剪（gradient clipping）
                # 限制梯度的范数：将模型所有参数的梯度的范数（L2范数）限制在args.max_grad_norm指定的最大值内
                torch.nn.utils.clip_grad_norm_(model.parameters(), args.max_grad_norm)
                # 梯度计算在所有GPU上进行，但参数更新只在主设备上执行一次
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating", mininterval=60):
        ...
        # 使用torch.no_grad()创建一个上下文管理器，在此上下文中的所有操作都不会计算梯度。
        with torch.no_grad():
            inputs = {
                "input_ids": batch[0],
                "attention_mask": batch[1],
        ...
        for i, feature_index in enumerate(feature_indices):
            ...
            # 把模型输出的结果存储到 all_results 列表中
            # 模型输出：[start_logits, end_logits]
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    # args.local_rank == -1 表示非分布式训练
    # args.local_rank == 0 表示分布式训练中的主进程
    # args.local_rank > 0 表示分布式训练中的其他辅助进程
        #TODO: 确保只有第一个进程在分布式训练中处理数据集，其他进程将使用缓存
        # PyTorch 分布式训练库中的一个同步原语，它的主要作用是在分布式训练环境中同步所有进程
        torch.distributed.barrier()


    # Load data features from cache or dataset file
    # 特征文件的含义：预处理数据集（如分词、编码等）是耗时操作，通过缓存可以避免每次训练都重复处理
    # 这段代码是用来构建缓存特征文件(cached features file)的路径的。
    # 特征文件是模型训练或评估时使用的预处理后的数据，
    # 缓存这些特征可以加速后续的训练运行。让我来解释这段代码的具体含义：
...
def main():
    ...
    if args.local_rank == 0:
        # Make sure only the first process in distributed training will download model & vocab
        # 之前在另一个同步点等待的所有非主进程会被释放，继续执行后续代码。
        torch.distributed.barrier()
...
#             raise ImportError("Please install apex from https://www.github.com/nvidia/apex to use fp16 training.")
    ...
    if args.do_train:
        ...
        # ___________________________________________________________
        global_step, tr_loss = train(args, train_dataset=train_dataset,model=model,tokenizer=tokenizer)
    ...
    # 一个线性层预测答案的开始位置
    # 一个线性层预测答案的结束位置
    # 这两个线性层的输入维度是BERT隐藏层维度(通常768)，输出维度是序列长度
    return results
