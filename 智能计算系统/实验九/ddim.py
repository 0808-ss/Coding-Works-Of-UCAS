...
class DDIMSampler(object):
    def __init__(self, model, schedule="linear", device=torch.device("mlu"), **kwargs):
        ...
        与原始扩散模型关系：
        - DDIM是DDPM的改进版本，保持相同的前向过程，但改进了反向采样
        - 继承DDPM的噪声调度和累积alpha参数
        
        Args:
            model: 预训练的扩散模型（通常是UNet）
            schedule: 噪声调度方式，默认"linear"
            device: 计算设备，这里使用MLU（机器学习单元）
        """
    ...
    def make_schedule(self, ddim_num_steps, ddim_discretize="uniform", ddim_eta=0.,verbose=True):
        ...
        to_torch = lambda x: torch.tensor(x, dtype=torch.float32, device=self.model.device) if not isinstance(x, torch.Tensor) else x.clone().detach().to(torch.float32).to(self.model.device)
        ...
        self.register_buffer('ddim_sqrt_one_minus_alphas', to_torch(np.sqrt(1. - ddim_alphas.cpu())))
    ...
    def sample(self,
        ...
        与原始扩散模型关系：
        - 这是用户调用的主接口，封装了完整的DDIM采样流程
        - 相比DDPM采样，可以用更少的步数（S）达到相似效果

        Args:
            S: DDIM采样步数（远小于DDPM的1000步）
            batch_size: 批次大小
            shape: 生成图像形状 (C, H, W)
            conditioning: 条件信息（文本嵌入等）
        """
    ...
    def ddim_sampling(self, cond, shape,
        ...
        if timesteps is None:
            # 选择使用DDPM完整步数还是DDIM稀疏步数
            # 例如 [0, 1, 2, ..., 999] 这是DDPM完整步数
            # 例如 [0, 50, 100, ..., 950]（20步） 这是DDIM稀疏步数
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False, temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None, unconditional_guidance_scale=1., unconditional_conditioning=None, dynamic_threshold=None):
        ...
        与原始扩散模型关系：
        - 这是DDIM的核心创新：确定性采样公式
        - 相比DDPM的随机采样，DDIM可以实现确定性生成
        - 实现了论文中的公式：x_{t-1} = √ᾱ_{t-1} * pred_x0 + √(1-ᾱ_{t-1}-σ²_t) * ε_θ + σ_t * z
        """
        ...
        ### 这里似乎有问题？？？
        # dir_xt = (a_prev.sqrt() - sigma_t * sigma_t / sqrt_one_minus_at) * pred_x0
        dir_xt = (1 - a_prev - sigma_t).sqrt() * e_t
        ...
        # 7. DDIM采样公式
        # x_{t-1} = √α_{t-1} * pred_x0 + dir_xt + σ_t * z
